...
main = do
  ...
  h1 <- spawnPipe "xmobar ~/.xmonad/xmobar1.hs"
  hPutStrLn h1 ""
...
manageHook' = composeAll
              ...
              , className     =? "MPlayer"    --> ask >>= doF . SS.sink
...
keys' conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
    ...
    -- focus
    , ((modMask,               xK_j     ), windows SS.focusDown)
    , ((modMask,               xK_k     ), windows SS.focusUp)
    , ((modMask,               xK_m     ), windows SS.focusMaster)

    -- swapping
    --, ((modMask .|. shiftMask, xK_Return), windows SS.swapMaster)
    , ((modMask .|. shiftMask, xK_j     ), windows SS.swapDown  )
    , ((modMask .|. shiftMask, xK_k     ), windows SS.swapUp    )
    ...
    [((m .|. modMask, k), windows $ f i)
        ...
        , (f, m) <- [(SS.view, 0), (SS.shift, shiftMask), (SS.greedyView, controlMask)]]
